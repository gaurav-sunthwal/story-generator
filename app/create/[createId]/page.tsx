...
const setting = [
  "A feline chef, imbued with magical cooking skills, prepares a nocturnal feast under a full moon, her every stir imbuing the dish with an ancient power.",
  "In a dimly lit kitchen, a cat in chef's attire uses magical ingredients to cook a powerful potion, stirring it with a focused gaze under the mystical glow of the night.",
  "A nocturnal cat, a master of culinary magic, creates a dish of immense power, the kitchen aglow with enchanted flames as they taste their creation.",
  "A mystical cat chef, cloaked in the shadows of the night, channels their power into a magical stew, the aroma filling the air with an enchanting spell.",
  "Under the cloak of night, a powerful cat, adorned in a chef's hat, conducts a magical cooking ritual, transforming ordinary ingredients into a source of unimaginable power.",
  "A ginger cat, a powerful sorcerer in disguise, brews a magical concoction in the twilight, their culinary skills masking a hidden nocturnal world of potent spells.",
];
...
const SelectSetting: React.FC<MoodSelectionComponentProps> = ({
  setting,
  selectSeting,
  setSelectSetting,
}) => (
        ...
        >
          <Card
            className={cn(
              "p-4 cursor-pointer transition-all hover:shadow-lg bg-gradient-to-r",
              setting.color,
              selectSeting?.includes(setting.id)
                ? "ring-2 ring-primary"
                : "hover:ring-1 hover:ring-primary/50"
            )}
            onClick={() => {
              setSelectSetting((prev: string[] | null) =>
                prev?.includes(setting.id)
                  ? prev.filter((id) => id !== setting.id)
                  : [...(prev || []), setting.id]
              );
            }}
          >
            <h4 className="font-semibold text-center">
              {capitalize(setting.id)}
            </h4>
          </Card>
        </motion.div>
      ))}
