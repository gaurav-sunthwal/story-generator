...
const FileUploadComponent: React.FC<FileUploadComponentProps> = ({previews, handleFileUpload, removeFile}) => (
    previews.length === 0 ? (
...
const StoryLengthSelectionComponent: React.FC<StoryLengthSelectionComponentProps> = ({storyLength, setStoryLength}) => (
    <div className="space-y-4">
...
function generateRandomColor(): string {
    const generateHex = () => Math.floor(Math.random() * 256).toString(16).padStart(2, '0');
    ...
    const handleNext = () => {
        if (step === 1) {
            ...
            StoryGenerator.generateThemes(files[0]).then((response) => {
                ...
                setTheme(response.data.data.themes.map((theme: string) => ({
                    id: theme,
            ...
            const storyData = {files, description, selectedMood, storyLength, color};
